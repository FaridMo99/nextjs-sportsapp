...
export async function message(prevState: unknown, formData: FormData) {
  try {
    const userMessage = formData.get("message");
    const historyUnknown = formData.get("history");
    const history = historyUnknown ? JSON.parse(historyUnknown.toString()) : [];
    const messages = [
      {
        role: "system",
        content: `
